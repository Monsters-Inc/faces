...
def Convolution(input_tensor,filters):
    ...
    x = Conv2D(filters=filters,kernel_size=(3, 3),padding = 'same',strides=(1, 1),kernel_regularizer=l2(0.001), activation='relu')(input_tensor)
    x = Dropout(0.1)(x)
    return x
...
def train_model(X_train, X_test, Y_train_2, Y_test_2):
    ...
    fle_s='Model_two_branches.h5'
    checkpointer = ModelCheckpoint(fle_s, monitor='val_loss',verbose=1,save_best_only=True,save_weights_only=False, mode='auto',save_freq='epoch')
    Early_stop=tf.keras.callbacks.EarlyStopping(patience=75, monitor='val_loss',restore_best_weights=True),
    callback_list=[checkpointer,Early_stop]

    Model.fit(X_train,Y_train_2,batch_size=64,validation_data=(X_test,Y_test_2),epochs=50,callbacks=[callback_list])
    return Model